...
for (let i = 0; i < plaatjes.length; i++) {
    ...
    plaatjes[i].addEventListener('click', geefInfo);
    plaatjes[i].addEventListener('dblclick', addLikes);
}
...
function geefInfo() {
    document.getElementById(this.title).firstElementChild.style.border = '6px solid' + this.dataset.kleur;
    document.getElementById('titel').innerText = this.title;
    document.getElementById('informatie').innerHTML = this.dataset.info;
    document.getElementById('info').style.border = '6px solid' + this.dataset.kleur;
}
...
function addLikes() {
    document.getElementById(this.title).dataset.clicks = this.dataset.clicks ++;
    document.getElementById(this.title).lastElementChild.innerHTML = this.dataset.clicks;
    var div = document.createElement('div');
    div.setAttribute('style', 'background-color: ' + this.dataset.kleur);
    div.setAttribute('class', 'bolletje');
